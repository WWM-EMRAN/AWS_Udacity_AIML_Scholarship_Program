...
#     print(filenames)
    pet_labels = [v[0] for v in list(results_dic.values())] 
#     classifier_labels = [] 
#     image_classification = classifier(test_image, model)
    for file_n in filenames:
        complete_file_n = f'{images_dir}/{file_n}'
        image_classification = classifier(complete_file_n, model) 
        image_classification = image_classification.lower().strip() 
#         classifier_labels.append(image_classification) 
        is_matched = 0 
        if results_dic[file_n][0] in image_classification:
            is_matched = 1 
        else:
            is_matched = 0 
#         print(complete_file_n, '||', results_dic[file_n], '==>', image_classification)
#         results_dic[file_n] = [ results_dic[file_n], image_classification, is_matched ]
        results_dic[file_n].extend([ image_classification, is_matched ])
