...
def train_one_epoch(train_dataloader, model, optimizer, loss):
    ...
    for batch_idx, (data, target) in tqdm(
        ...
        # # YOUR CODE HERE: -- CrossEntropyLoss does not have this backward function 
        loss_value.backward()  
...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    scheduler  = ExponentialLR(optimizer, gamma=0.99) # YOUR CODE HERE
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if (valid_loss_min - valid_loss) / valid_loss_min > 0.01:
            ...
            torch.save(model.state_dict(), save_path)  
            print(f'Model file Saved successfully {save_path}...') 
                
            # if not os.path.exists(save_path):
            #     print(f'Model file does not exist {save_path}...') 
            #     torch.save(model.state_dict(), save_path)  
            #     print(f'Model file Saved successfully {save_path}...') 
            # else:
            #     print(f'Model file already exist {save_path}...') 
...
def one_epoch_test(test_dataloader, model, loss):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            pred  = logits.data.max(1, keepdim=True)[1] #torch.max(logits, dim=1)[1] # torch.softmax(logits, dim=1) # YOUR CODE HERE
